...
namespace CESupport
{
    public static class PathStatWorker_MeleeDamage2
    {
        public static bool GetExplanationUnfinalizedPrefix(ref string __result, StatRequest req, ToStringNumberSense numberSense)
        {
            ...
            if (comp != null)
            {
                if (!comp.part.NullOrEmpty<Thing>())
                {
                    foreach(Thing t in comp.part)
                    {
                        ...
                        stringBuilder.AppendLine(DefDatabase<StatDef>.GetNamed("MeleeDamage").Worker.GetExplanationUnfinalized(StatRequest.For(t), numberSense));
                    }
            ...
            if(!((thingDef != null) ? thingDef.tools : null).NullOrEmpty<Tool>())
            {
                ...
                foreach (Tool tool in list)
                {
                    ...
                    stringBuilder.AppendLine(string.Format("    " + "FinalValue".Translate() + ": {0} - {1}", (adjustedDamage * num).ToStringByStyle(ToStringStyle.FloatMaxTwo, ToStringNumberSense.Absolute), (adjustedDamage * num2).ToStringByStyle(ToStringStyle.FloatMaxTwo, ToStringNumberSense.Absolute)));
                    stringBuilder.AppendLine();
                }
                __result = stringBuilder.ToString();
                return false;
